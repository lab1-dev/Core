...
import {DateTime} from "../CoreExports";
...
export class TimeSpan {
    ...
    public static get now():TimeSpan{
    ...
    public static fromString(text: string): TimeSpan {
        if (!text) {
    ...
    public get totalHours(): number {
        return Math.round(this.msSinceEpoch * hoursPerMS);
    }
    ...
    public toDuration():Duration{
        return Duration.fromObject({days:this.days, hours:this.hours,minutes:this.minutes, seconds:this.seconds, milliseconds:this.milliseconds});
    }

    /**
     * Format the TimeSpan as time format
     * @param amPM Display time as 12 hours with AM/PM (only if day is zero)
     */
    public toString(amPM: boolean = false): string {
    ...
    public multiply(factor:number): TimeSpan {
        return new TimeSpan(this.msSinceEpoch*factor);
    }
